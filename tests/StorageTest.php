...
final class StorageTest extends TestCase
{
    ...
    {
        $path = $this->tempDir . '/preview.json';
        $payload = [
            'preview'=> [
                'date' => '2026-05-31',
                'stadium_number' => 6,
        ...
        $this->assertArrayHasKey('preview', $savedPayload);
    ...
    {
        $path = $this->tempDir . '/nested/dir/preview.json';
        $payload = ['preview' => ['date' => '2026-05-31']];
    ...
    public function testSaveOverwritesExistingFile(): void
    {
        ...
        Storage::save($path, ['preview' => ['race_number' => 1]]);
        Storage::save($path, ['preview' => ['race_number' => 2]]);

        $savedPayload = json_decode(file_get_contents($path), true);
        $this->assertSame(2, $savedPayload['preview']['race_number']);
    }
